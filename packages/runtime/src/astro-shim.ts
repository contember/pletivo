...
// Render-pass tracking lives in ./render-context so the JSX runtime can
// reach it without importing this module. Re-exported for existing callers.
export { runWithRenderTracking, pushTsxStyle } from "./render-context";
...
export function createComponent(
  ...
  const wrapped = async (
    ...
    if (moduleId) recordRenderedModule(moduleId);
...
export function renderScript(_result: AstroResult, id: string): HtmlString {
  ...
  if (!src) return createHtml("");
  return createHtml(
    `<script type="module" src="${withBase(src)}"></script>`,
  );
}
