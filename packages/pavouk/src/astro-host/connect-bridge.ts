 ...
 *
 * Pavouk's dev server is Bun.serve which speaks Fetch API Request/Response.
 ...
 *     the optional `finalHandler` callback to obtain pavouk's own
...
export async function dispatchMiddlewares(
  ...
  const next = async (err?: unknown): Promise<void> => {
    ...
    if (!mw) {
      // End of middleware chain. Run pavouk's final handler, then pipe
  ...
  if (chainError) {
    console.error("[pavouk-astro-host] middleware error:", chainError);
