/**
 * Astro compiler runtime shim for pavouk.
 *
 ...
 *
 * The shim maps Astro's tagged-template rendering model onto pavouk's
...
export async function renderComponent(
  ...
  // Plain function component (pavouk JSX, or shim Fragment)
  if (typeof Component === "function") {
    // Bridge Astro slots to pavouk's `children` convention
 ...
 * pavouk's `dev.ts` and `build.ts` when the page module is a `.astro` file.
 */
