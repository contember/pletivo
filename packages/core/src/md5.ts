 ...
 * text (`css.ts`), and an image's from the same hasher over its bytes, so those
 * names only agree between the two hosts if the same digest comes out here.
 *
 ...
 * code paths, and `packages/workers/test/md5.test.ts` holds it to `Bun.CryptoHasher`
 * byte for byte.
 *
 * It lives in `@pletivo/core` rather than in the Workers host because both hosts name
 * files with it — the Bun host through `Bun.CryptoHasher` today, and every host
 * through this when the input is bytes that never touched a filesystem.
 */
...
/** MD5 of a UTF-8 string or of raw bytes, lowercase hex. */
export function md5Hex(input: string | Uint8Array): string {
  const view = padded(typeof input === "string" ? new TextEncoder().encode(input) : input);
